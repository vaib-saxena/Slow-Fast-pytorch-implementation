...
class SlowFast(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], class_num=10, dropout=0.5 ):
        ...
        self.fast_res5 = self._make_layer_fast_last(
            Bottleneck2, 64, layers[3], stride=1, head_conv=3)
        ...
        self.slow_res5 = self._make_layer_slow_last(
            Bottleneck2, 512, layers[3], stride=1, head_conv=3)
    ...
    def SlowPath(self, input, lateral):
        #print("input_shape:",np.shape(input))
    ...
    def _make_layer_fast_last(self, block, planes, blocks, stride=1, head_conv=1):
        downsample = None
        if stride != 1 or self.fast_inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv3d(
    ...
    def _make_layer_slow_last(self, block, planes, blocks, stride=1, head_conv=1):
        # print('_make_layer_slow',planes)
        downsample = None
        if stride != 1 or self.slow_inplanes != planes * block.expansion:
            # print('self.slow_inplanes',self.slow_inplanes)
...
if __name__ == "__main__":
    num_classes = 101
    input_tensor = torch.autograd.Variable(torch.rand(16, 3, 64, 112, 112)).cuda()
    model = resnet50(class_num=num_classes).cuda()
